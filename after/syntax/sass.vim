...
if exists('b:current_syntax')
    if b:current_syntax == "sass"
        syntax clear    " clear the old syntax
    elseif b:current_syntax == "fixed_sass"
        finish
    endif
endif
...
syn match sassProperty "\%(^\s*\)\@<=\s\%(\%([[:alnum:]-]\|#{[^{}]*}\)\+\s*:\|:[[:alnum:]-]\+\)"hs=s+1 contains=css.*Prop skipwhite nextgroup=sassCssAttribute
syn match sassProperty "\%(^\s*\)\@<=\s\%(:\=[[:alnum:]-]\+\s*=\)"hs=s+1 contains=css.*Prop skipwhite nextgroup=sassCssAttribute
...
syn match sassMixing "\%(^\s\+\)\@<=+"        nextgroup=sassMixinName
...
let b:current_syntax = "fixed_sass"
